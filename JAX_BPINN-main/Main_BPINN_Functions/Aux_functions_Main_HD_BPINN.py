...
def analytic_solution (x, tau, ALPHA, LAMBDA_FACTOR):
    solution = jnp.exp( -jnp.pi**2 * ALPHA * (tau / LAMBDA_FACTOR)) * jnp.sin(jnp.pi *x)
    return solution
...
def get_data (main_key):
    ...
    batch = {
        ...
        "u_ic": u_ic, "u_bc_l": u_bc_l, "u_bc_r": u_bc_r, "u_pde": u_pde,
        }
...
def res_pde_HD (model, x, t, sampled_params):
    ALPHA = 0.5
    LAMBDA_FACTOR = 1
    u,du_dx,du_dt,du_dxx = model.out_u_pde(x,t,sampled_params)
    residuum = du_dt - (ALPHA / LAMBDA_FACTOR) * du_dxx - 0
    return residuum

def res_bc_HD (model, x, t, sampled_params):
    u, _, _ , _ = model.out_u_only(x,t,sampled_params)
    residuum  = u - 0
    return residuum

def res_ic_HD (model, x, t, sampled_params):
    u, _, _ , _ = model.out_u_only(x,t,sampled_params)
...
def single_sample_loss (model, batch_data, local_lambdas, global_lambdas, local_lambda_tech, sigma_likelihood, fwd_key, GAMMA_RBA, LR_RBA):
    ...
    res_pde_pre = res_pde_HD(model, x_pde, t_pde, sampled_params)

    res_bc_l_pre = res_bc_HD (model, x_bc_l,t_bc_l,sampled_params)

    res_bc_r_pre = res_bc_HD (model, x_bc_r, t_bc_r,sampled_params)

    res_ic_pre = res_ic_HD(model, x_ic, t_ic, sampled_params)
