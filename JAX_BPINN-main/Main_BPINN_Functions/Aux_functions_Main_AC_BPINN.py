...
def get_data (main_key):
    ...
    mat_data = scipy.io.loadmat('./Main_BPINN_Functions/Data/AC.mat')

    t_mat = mat_data['tt'].flatten()[:,None]
    x_mat = mat_data['x'].flatten()[:,None]
    Exact_mat = np.real(mat_data['uu']).T
...
def define_sigma_likelihood ():
    sigma_likelihood = {
        ...
        # 'SIGMA_bc_u' : jnp.array(1.0/(2.0*jnp.pi*1000.0),dtype=jnp.float32), 
        # 'SIGMA_bc_du_dx' : jnp.array(1.0/(2.0*jnp.pi*1000.0),dtype=jnp.float32) 

        'SIGMA_ic'       : jnp.array(0.005,dtype=jnp.float32), 
        'SIGMA_pde'      : jnp.array(0.001,dtype=jnp.float32),  
        'SIGMA_bc_u'     : jnp.array(0.005,dtype=jnp.float32), 
        'SIGMA_bc_du_dx' : jnp.array(0.005,dtype=jnp.float32) 
    } 
...
def initialize_local_lambdas(batch_data):
    local_lambdas ={
        ...
        "lambda_bc_u"     : jnp.ones_like(batch_data["x_bc_l"].shape[0], dtype=jnp.float32),
        "lambda_bc_du_dx" : jnp.ones_like(batch_data["x_bc_l"].shape[0], dtype=jnp.float32),
...
def initialize_global_lambdas (IC_VALUE):
    global_lambdas = {
        ...
        "lambda_lik_bc_u"     : jnp.array(1.0, dtype=jnp.float32),
        "lambda_lik_bc_du_dx" : jnp.array(1.0, dtype=jnp.float32),
...
def res_pde (model, x, t, sampled_params):
    u,du_dx,du_dt,du_dxx = model.out_u_pde(x,t,sampled_params)
    residuum = du_dt - 0.0001 * du_dxx - 5 * (u - u**3)
    return residuum

def res_ic (model, x, t, sampled_params):
    u, _, _ , _ = model.out_u_only(x,t,sampled_params)
    residuum = u - initial_analytic_condition(x)
    return residuum

def u_and_du_dx_BC (model, x, t, sampled_params):
    u, du_dx, _, _ = model.out_u_pde(x,t,sampled_params)    
    return u, du_dx

# def res_bc (model, x1, t1, x2, t2, fwd_key):
#     key_1, key_2 = jax.random.split(fwd_key,2)
#     u_1, du_dx_1 = u_and_du_dx_BC(model,x1,t1,key_1)
#     u_2, du_dx_2 = u_and_du_dx_BC(model,x2,t2,key_2)
#     res_u_bc = u_1 - u_2
#     res_du_dx_bc = du_dx_1 - du_dx_2
#     return res_u_bc, res_du_dx_bc

def res_bc_u (model, x1, t1, x2, t2, sampled_params):
    u_1, _ = u_and_du_dx_BC(model,x1,t1,sampled_params)
    u_2, _ = u_and_du_dx_BC(model,x2,t2,sampled_params)
    res_u_bc = u_1 - u_2
    return res_u_bc

def res_bc_du_dx (model, x1, t1, x2, t2, sampled_params):
    _ , du_dx_1 = u_and_du_dx_BC(model,x1,t1,sampled_params)
    _ , du_dx_2 = u_and_du_dx_BC(model,x2,t2,sampled_params)
    res_du_dx_bc = du_dx_1 - du_dx_2
    return res_du_dx_bc
...
def single_sample_loss (model, batch_data, local_lambdas, global_lambdas, local_lambda_tech, sigma_likelihood, fwd_key, GAMMA_RBA, LR_RBA):
    ...
    res_pde_pre = res_pde(model, x_pde, t_pde, sampled_params)

    res_bc_u_pre = res_bc_u (model, x_bc_l,t_bc_l, x_bc_r, t_bc_r,sampled_params)

    res_bc_du_dx_pre = res_bc_du_dx (model, x_bc_l,t_bc_l, x_bc_r, t_bc_r,sampled_params)

    res_ic_pre = res_ic(model, x_ic, t_ic, sampled_params)
    ...
    if local_lambda_tech == 'Residual_RBA':
        ...
        new_res_bc_u, new_local_lambda_bc_u = RBA_apply(res_bc_u_pre,local_lambdas['lambda_bc_u'],GAMMA_RBA,LR_RBA,EPS)
        new_res_bc_du_dx, new_local_lambda_bc_du_dx = RBA_apply(res_bc_du_dx_pre,local_lambdas['lambda_bc_du_dx'],GAMMA_RBA,LR_RBA,EPS)
        ...
        sampled_log_lik_bc_u     = gaussian_log_likelihood_RBA(new_res_bc_u, sigma_likelihood['SIGMA_bc_u'],new_local_lambda_bc_u)
        sampled_log_lik_bc_du_dx = gaussian_log_likelihood_RBA(new_res_bc_du_dx, sigma_likelihood['SIGMA_bc_du_dx'],new_local_lambda_bc_du_dx)
        ...
        unaltered_log_lik_bc_u     = gaussian_log_likelihood(res_bc_u_pre, sigma_likelihood['SIGMA_bc_u'])
        unaltered_log_lik_bc_du_dx = gaussian_log_likelihood(res_bc_du_dx_pre, sigma_likelihood['SIGMA_bc_du_dx'])
    ...
    #     new_res_ic, new_local_lambda_ic = SA_apply(res_ic_pre,local_lambdas_SA.lambda_ic)
        ...
        sampled_log_lik_bc_u, new_local_lambda_bc_u         = likelihood_RBA(res_bc_u_pre, sigma_likelihood['SIGMA_bc_u'], local_lambdas['lambda_bc_u'], GAMMA_RBA, LR_RBA, EPS)
        sampled_log_lik_bc_du_dx, new_local_lambda_bc_du_dx = likelihood_RBA(res_bc_du_dx_pre, sigma_likelihood['SIGMA_bc_du_dx'], local_lambdas['lambda_bc_du_dx'], GAMMA_RBA, LR_RBA, EPS)
        ...
        new_res_bc_u, new_local_lambda_bc_u = res_bc_u_pre, local_lambdas['lambda_bc_u']
        new_res_bc_du_dx, new_local_lambda_bc_du_dx = res_bc_du_dx_pre, local_lambdas['lambda_bc_du_dx']
        ...
        sampled_log_lik_bc_u     = gaussian_log_likelihood(new_res_bc_u, sigma_likelihood['SIGMA_bc_u'])
        sampled_log_lik_bc_du_dx = gaussian_log_likelihood(new_res_bc_du_dx, sigma_likelihood['SIGMA_bc_du_dx'])
        sampled_log_lik_ic       = gaussian_log_likelihood(new_res_ic, sigma_likelihood['SIGMA_ic'])

        unaltered_log_lik_pde      = sampled_log_lik_pde
        unaltered_log_lik_bc_u     = sampled_log_lik_bc_u
        unaltered_log_lik_bc_du_dx = sampled_log_lik_bc_du_dx
    ...
    return ({'log_lik_pde':sampled_log_lik_pde,
            ...
            'log_lik_bc_u':sampled_log_lik_bc_u,
            'log_lik_bc_du_dx':sampled_log_lik_bc_du_dx,
            ...
            'unaltered_log_lik_bc_u':unaltered_log_lik_bc_u,
            'unaltered_log_lik_bc_du_dx':unaltered_log_lik_bc_du_dx},
            {'lambda_pde':new_local_lambda_pde,
            'lambda_bc_u':new_local_lambda_bc_u,
            'lambda_bc_du_dx':new_local_lambda_bc_du_dx,
...
def loss_terms (model, batch_data, local_lambdas, global_lambdas, local_lambda_tech, sigma_likelihood, fwd_key, K_SAMPLES, GAMMA_RBA, LR_RBA):
    ...
    vmapped_fn = jax.vmap(
        lambda k: single_sample_loss(model, batch_data, local_lambdas, global_lambdas, local_lambda_tech, 
    ...
    total_log_lik_bc_u = jnp.mean(sampled_results['log_lik_bc_u'])
    total_log_lik_bc_du_dx = jnp.mean(sampled_results['log_lik_bc_du_dx'])
    ...
    unaltered_total_log_lik_bc_u = jnp.mean(sampled_results['unaltered_log_lik_bc_u'])
    unaltered_total_log_lik_bc_du_dx = jnp.mean(sampled_results['unaltered_log_lik_bc_du_dx'])
    ...
    total_log_likelihood = (
        ...
        global_lambdas['lambda_lik_bc_u'] * total_log_lik_bc_u +
        global_lambdas['lambda_lik_bc_du_dx'] * total_log_lik_bc_du_dx +
    ...
    log_loss = {
        ...
        'log_lik_bc_u':total_log_lik_bc_u,
        'log_lik_bc_du_dx':total_log_lik_bc_du_dx,
        'unaltered_log_lik_pde': unaltered_total_log_lik_pde,
        'unaltered_log_lik_bc_u':unaltered_total_log_lik_bc_u,
        'unaltered_log_lik_bc_du_dx':unaltered_total_log_lik_bc_du_dx,
    ...
    new_local_lambdas = {
        ...
        'lambda_bc_u':jnp.mean(sampled_local_lambdas['lambda_bc_u'],axis=0),
        'lambda_bc_du_dx':jnp.mean(sampled_local_lambdas['lambda_bc_du_dx'],axis=0),
...
def make_step_None (model, optimizer1, state1, batch_data, local_lambdas, global_lambdas, local_lambda_tech, sigma_likelihood, fwd_key, K_SAMPLES, GAMMA_RBA, LR_RBA):
    ...
    (loss, (loss_log, new_local_lambdas)), grads = eqx.filter_value_and_grad(loss_terms, has_aux = True)(model, batch_data, local_lambdas, global_lambdas, local_lambda_tech, sigma_likelihood, fwd_key, K_SAMPLES, GAMMA_RBA, LR_RBA)
...
def logs_init ():
    log = {
    "negELBO_history"            : [],
    "log_lik_bc_u_history"            : [],
    "log_lik_bc_du_dx_history"        : [],
    "log_lik_ic_history"              : [],
    "log_lik_pde_history"             : [],
    "unaltered_log_lik_pde": [],
    "unaltered_log_lik_ic": [],
    "unaltered_log_lik_bc_u": [],
    "unaltered_log_lik_bc_du_dx": [], 
    "log_prior_history"             : [],
    "log_var_posterior_history"             : [],
    "lambda_g_pde_history"        : [],
    "lambda_g_ic_history"       : [],
    "lambda_g_bc_u_history"     : [],
    "lambda_g_bc_du_dx_history" : [],
    "lambda_g_prior_history" : [],
    "lambda_g_var_posterior_history" : [],
    "lambda_l_pde_history"        : [],
    "lambda_l_ic_history"       : [],
    "lambda_l_bc_u_history"     : [],
    "lambda_l_bc_du_dx_history" : [],
...
def update_logs (logs, logs_history, global_lambdas, local_lambdas):
    ...
    logs_history["log_lik_bc_u_history"].append(float(logs["log_lik_bc_u"]))
    logs_history["log_lik_bc_du_dx_history"].append(float(logs["log_lik_bc_du_dx"]))
    ...
    logs_history["unaltered_log_lik_bc_u"].append(float(logs["unaltered_log_lik_bc_u"]))
    logs_history["unaltered_log_lik_bc_du_dx"].append(float(logs["unaltered_log_lik_bc_du_dx"]))
    ...
    logs_history["lambda_g_bc_u_history"].append(float(global_lambdas["lambda_lik_bc_u"]))
    logs_history["lambda_g_bc_du_dx_history"].append(float(global_lambdas["lambda_lik_bc_du_dx"]))
    ...
    logs_history["lambda_l_bc_u_history"].append(float(jnp.mean(local_lambdas["lambda_bc_u"])))
    logs_history["lambda_l_bc_du_dx_history"].append(float(jnp.mean(local_lambdas["lambda_bc_du_dx"])))
...
def predictive_samples (model, x, t, sample_key, n_samples = 25):
    ...
    def single_sample (key):
        u, _, _, _ , _, _ = model.out_u_only (x,t,key)
        return u.squeeze()
